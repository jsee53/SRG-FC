...
// 개인정보 화면의 "베스트 플레이어 받은 횟수" 집계와 일정 탭의 팀별 투표 UI가 이 데이터를 같이 씀
export function useBestPlayerVotes() {
  ...
  const castVote = useCallback(async (eventId, attendeeId) => {
  ...
  const retractVote = useCallback(async (eventId) => {
    const { error: rpcError } = await supabase.rpc('retract_best_player_vote', { target_event_id: eventId })
    if (!rpcError) {
      await refetch()
    }
    return { error: rpcError }
