...
export default function MemberDetail({
  ...
  // 아래로 당기는 동안 손가락을 따라 시트가 같이 내려가도록 실시간으로 반영
  // (좌우 스와이프는 이전/다음 카드 전환이라 여기서는 따로 안 따라가고 손 뗄 때 바로 전환)
  ...
  function handleTouchEnd(e) {
    ...
    const isSwipeDown = dy > 0 && dy > Math.abs(dx) * 1.5 && startScrollTop <= 0
    if (!isSwipeDown || !sheetRef.current) return
        ...
        <div
          ...
          className={
            !isNavigating
              ? ''
              : direction === 'next'
                ? '[animation:slide-next-in_0.55s_cubic-bezier(0.22,1,0.36,1)]'
                : '[animation:slide-prev-in_0.55s_cubic-bezier(0.22,1,0.36,1)]'
          }
        >
