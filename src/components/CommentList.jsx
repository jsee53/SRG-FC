...
export default function CommentList({ comments, members, session, isAdmin, onDelete, onToggleLike, onRequireLogin }) {
  if (comments.length === 0) {
    return <p className="py-4 text-center text-sm text-[var(--color-text-muted)]">아직 댓글이 없어요.</p>
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      {comments.map((comment) => {
        ...
        return (
          <div key={comment.id} className="rounded-xl bg-[var(--color-surface-faint)] p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              {canDelete && (
                <button
                  ...
                  className="text-xs text-[var(--color-text-faint)] hover:text-red-400"
                >
                  삭제
                </button>
              )}
            </div>
            <p className="mt-1 whitespace-pre-wrap text-sm text-[var(--color-text-soft)]">{comment.content}</p>
            <button
              type="button"
              onClick={() => handleToggleLike(comment, isLiked)}
              className={`mt-1 text-xs transition-colors ${isLiked ? 'text-rose-400' : 'text-[var(--color-text-faint)] hover:text-rose-300'}`}
            >
